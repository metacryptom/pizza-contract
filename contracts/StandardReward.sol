...
contract StandardReward is IRewarder,Ownable {
	...
	constructor(
		...
		require(Address.isContract(address(_PARK_EXT)), "StandardReward: PIZZAParkExt  must be a valid contract");
	...
	function onPIZZAReward(
		uint256 pid,
		address _user,
		uint256 lpToken,
		uint256 _pendingPIZZA
	...
	function pendingToken(uint256 _pid, address _user,uint256 _pendingPIZZA) external view override returns (IERC20 token,uint256 pending) {
